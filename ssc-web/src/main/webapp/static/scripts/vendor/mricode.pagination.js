...
(function (factory) {
    ...
    var Page = function (element, options) {
        var defaultOption = {
            ...
            showInfo: false,
            infoFormat: '当前显示为 {start} ~ {end} 条信息，共 {total} 条',
            noInfoText: '没有符合条件的记录，请更改查询条件',
            showJump: false,
            jumpBtnText: 'Go',
            showPageSizes: false,
        ...
        this.$page = $('<ul class="m-pagination-page m-pagination-page-' + keyWords + '"></ul>').hide();
        this.$size = $('<div class="m-pagination-size m-pagination-size-' + keyWords + '"></div>').hide();
        this.$jump = $('<div class="m-pagination-jump m-pagination-jump-' + keyWords + '"></div>').hide();
        this.$info = $('<div class="m-pagination-info m-pagination-info-' + keyWords + '"></div>').hide();
        ...
        this.currentPageIndex = utility.convertInt(this.options.pageIndex);//起始页为1
    ...
    Page.prototype = {
        ...
        initHtml: function () {
            ...
            //init jump module
            var jumpHtml = '<div class="m-pagination-group m-pagination-group-' + keyWords + '"><input data-page-btn="jump" type="text"><button data-page-btn="jump" type="button">' + this.options.jumpBtnText + '</button></div>';
        ...
        remote: function () {
            if (typeof this.options.remote.pageParams === 'function') {
                ...
                this.currentParams[this.options.remote.pageIndexName] = this.currentPageIndex + 1;
        ...
        getAjax: function (pagination, url, data, success, beforeSend, complate,traditional) {
            $.ajax({
                ...
                complete: function (XMLHttpRequest, textStatue) {
                    console.log(textStatue,'textStatue');
