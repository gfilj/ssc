...
public class SentinelRedisDaoHandler extends AbstractRedisDaoHandler implements RedisDaoStrategy{
	...
	public RedisDao getRedisDao(String id) {
		String fullName = Constant.SENTINEL_PREFIX + id;
        ...
        try {
    		...
    		String host = poolConfig.getHostPort().get(0).getHost() + ":" + poolConfig.getHostPort().get(0).getPort();
    		Set<String> sentinels = new HashSet<String>();  
            sentinels.add(host);  
            
            JedisSentinelPool pool = null;
    		if (StringUtils.isBlank(poolConfig.getPassword())) {
            	pool = new JedisSentinelPool(poolConfig.getMasterName(), sentinels, 
        	...
        	if (pool != null) {
                ...
                redisDao = new SentinelRedisDao(pool);
        		if (redisDao != null) {
        	...
        	e.printStackTrace();
        	logger.error("获取sentinel redis pool出错",e);
        	throw new RuntimeException("获取sentinel redis pool出错",e);
        } finally {
            lock.unlock();
        }
